...
private:
  ...
  int x;  // representing x position
  int y; // representing y position
  int l; // representing length of the animal
  int w; // representing width of the animal
...
public:
    animal(const animal& pa); // constructor to copy an animal existing
...
private :
    ...
    time_t timer = clock(); // represent the cooldown to reproduce
...
private :
    time_t timer = clock(); // represent the timer before dying of starvation
public :
    // Ctor
...
public :
    dog(SDL_Surface* window_surface_ptr, SDL_Event event);
...
private:
  ...
  ground ground = ground::ground(this->window_surface_ptr_);
